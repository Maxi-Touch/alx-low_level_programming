 ...
 *
 * Description: Note If the file is not an ELF file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
...
/**
 * print_version - Thsi prints the version of an ELF header.
 ...
 * @elf: Tdescriptor of the ELF file.
 *
 * Description: Note If the file cannot be closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * Return: 0 on success.
 *
 * Description: check If the file is not an ELF File or
 *              the function fails - then exit code 98.
 */
